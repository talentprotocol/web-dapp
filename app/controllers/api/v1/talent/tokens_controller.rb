  ...
  def update
    ...
    if token.update(token_params)
      if token.deployed? && !was_deployed
        ...
        UpdateQuestJob.perform_later(type: "Tasks::PublicProfile", user_id: current_user.id)
        UpdateQuestJob.perform_later(type: "Tasks::LaunchToken", user_id: current_user.id)
      end
