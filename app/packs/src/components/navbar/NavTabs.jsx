    ...
    if (url.pathname == tab.url) {
      return (
        <li key={`tab-${tab.url}`} className="nav-item mr-3">
          <a className="nav-link active tal-nav-item-active" href={tab.url}>
            <small>
      ...
      return (
        <li key={`tab-${tab.url}`} className="nav-item mr-3">
          <a className="nav-link tal-nav-item" href={tab.url}>
...
const DropdownOptions = ({ sortOptions, activeLabel }) => {
  return sortOptions.map((option) => {
    ...
    if (
      url.searchParams.toString().includes(option.param) ||
